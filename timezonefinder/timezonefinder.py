...
class AbstractTimezoneFinder(ABC):
    ...
    def __init__(
        ...
        # load the new mapping from hex id to unique zone id
        ...
        try:
            ...
            # For backwards compatibility with older data packages
    ...
    def unique_zone_id(self, *, lng: float, lat: float) -> Optional[int]:
        ...
        """
        hex_id = h3.latlng_to_cell(lat, lng, SHORTCUT_H3_RES)
        if self.unique_zone_mapping:
            return self.unique_zone_mapping.get(hex_id)

        # fallback to old logic if file was not there
        polys = self.get_boundaries_in_shortcut(lng=lng, lat=lat)
        if len(polys) == 0:
            return None
        if len(polys) == 1:
...
class TimezoneFinder(AbstractTimezoneFinder):
    ...
    def timezone_at(self, *, lng: float, lat: float) -> Optional[str]:
        ...
        # if a hex id is within a unique zone, we can immediately return the zone name
        if self.unique_zone_mapping:
        ...
        if last_zone_change_idx == 0:
            # this check is now redundant if the new `unique_zones.fbs` is used,
