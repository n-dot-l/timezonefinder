...
from timezonefinder.flatbuf.hex_zone_utils import read_hex_zones
...
class AbstractTimezoneFinder(ABC):
    ...
    def __init__(
        ...
        # Read the hex-to-zone mapping for single-zone shortcuts
        hex_zone_collection = read_hex_zones(self.data_location)
        self.hex_zone_mapping: Dict[int, int] = {}
        if hex_zone_collection:
            for i in range(hex_zone_collection.EntriesLength()):
                entry = hex_zone_collection.Entries(i)
                self.hex_zone_mapping[entry.HexId()] = entry.ZoneId()
    ...
    def get_boundaries_in_shortcut(self, *, lng: float, lat: float) -> Optional[np.ndarray]:
        """
        ...
        return self.shortcut_mapping.get(hex_id)
    ...
    def unique_zone_id(self, *, lng: float, lat: float) -> Optional[int]:
        ...
        """
        hex_id = h3.latlng_to_cell(lat, lng, SHORTCUT_H3_RES)
        zone_id = self.hex_zone_mapping.get(hex_id)
...
class TimezoneFinder(AbstractTimezoneFinder):
    ...
    def _load_hole_registry(self) -> Dict[int, Tuple[