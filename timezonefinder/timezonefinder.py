...
class AbstractTimezoneFinder(ABC):
    ...
    def __init__(
        ...
        path_to_unique_zones = get_unique_zone_file_path(self.data_location)
        if path_to_unique_zones.exists():
            self.unique_zone_mapping = read_unique_zones_binary(path_to_unique_zones)
...
class TimezoneFinderL(AbstractTimezoneFinder):
    ...
    def timezone_at(self, *, lng: float, lat: float) -> Optional[str]:
        ...
        """
        lng, lat = utils.validate_coordinates(lng, lat)
        # check for pre-computed unique zone
        hex_id = h3.latlng_to_cell(lat, lng, SHORTCUT_H3_RES)
        if self.unique_zone_mapping:
