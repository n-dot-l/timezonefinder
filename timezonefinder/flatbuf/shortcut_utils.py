...
def write_shortcuts_flatbuffers(
    shortcut_mapping: Dict[int, Tuple[List[int], int]], # Modified type hint to include unique_zone_id
    ...
    Args:
        shortcut_mapping: Dictionary mapping H3 hexagon IDs to (lists of polygon IDs, unique zone ID)
    ...
    for hex_id, (poly_ids, unique_zone_id) in shortcut_mapping.items(): # Unpack tuple
...
def read_shortcuts_binary(
    file_path: Path,
) -> Dict[int, Tuple[np.ndarray, int]]: # Modified return type to include unique_zone_id
    """
    ...
    Returns:
        Dictionary mapping H3 hexagon IDs to (numpy arrays of polygon IDs, unique zone ID)
    """
