...
def get_unique_zones_file_path(output_path: Path = DEFAULT_DATA_DIR) -> Path:
...
def write_unique_zones_flatbuffers(
    ...
    Returns:
        None
    """
    print(f"writing {len(unique_zone_mapping)} unique zones to binary file {output_file}")
    builder = flatbuffers.Builder(0)
    entry_offsets = []

    for hex_id, zone_id in unique_zone_mapping.items():
        # Start building unique entry
    ...
    # Create vector of unique entries
...
def read_unique_zones_binary(
    ...
    Args:
        file_path: Path to the unique zones FlatBuffer file.

    Returns:
        Dictionary mapping H3 hexagon IDs to unique zone ID
    """
