...
from timezonefinder.flatbuf.UniqueZoneShortcutEntry import (
    ...
    UniqueZoneShortcutEntryAddH3Id,
...
def get_unique_zone_shortcut_file_path(output_path: Path = DEFAULT_DATA_DIR) -> Path:
    ...
    return output_path / "unique_zone_shortcuts.uzsc"


def write_unique_zone_shortcuts_flatbuffers(
    unique_zone_shortcut_mapping: Dict[int, int],
    output_file: Path,
) -> None:
    """
    Write unique zone H3 shortcuts to a FlatBuffer binary file.

    Args:
        unique_zone_shortcut_mapping: Dictionary mapping H3 hexagon IDs to unique zone IDs
    ...
    """
    print(
        f"writing {len(unique_zone_shortcut_mapping)} unique zone shortcuts to binary file {output_file}"
    )
    builder = flatbuffers.Builder(0)
    entry_offsets = []

    for h3_id, zone_id in unique_zone_shortcut_mapping.items():
        # Start building unique zone shortcut entry
        UniqueZoneShortcutEntryStart(builder)
        UniqueZoneShortcutEntryAddH3Id(builder, h3_id)
        UniqueZoneShortcutEntryAddZoneId(builder, zone_id)
        entry_offsets.append(UniqueZoneShortcutEntryEnd(builder))

    # Create vector of unique zone shortcut entries
    ...
    # Create UniqueZoneShortcutCollection
    UniqueZoneShortcutCollectionStart(builder)
    UniqueZoneShortcutCollectionAddEntries(builder, entries_vector)
...
def read_unique_zone_shortcuts_binary(
    ...
    collection = UniqueZoneShortcutCollection.GetRootAsUniqueZoneShortcutCollection(
        buf, 0
    )

    unique_zone_shortcut_mapping = {}
    for i in range(collection.EntriesLength()):
        entry = collection.Entries(i)
        h3_id = entry.H3Id()
        zone_id = entry.ZoneId()
        unique_zone_shortcut_mapping[h3_id] = zone_id

    return unique_zone_shortcut_mapping
