...
import flatbuffers
import numpy as np
from timezonefinder.configs import DEFAULT_DATA_DIR, UNIQUE_ZONE_SHORTCUTS_FNAME
...
def get_unique_zone_shortcut_file_path(output_path: Path = DEFAULT_DATA_DIR) -> Path:
    ...
    return output_path / UNIQUE_ZONE_SHORTCUTS_FNAME
...
def write_unique_zone_shortcuts_flatbuffers(
    ...
    for hex_id, zone_id in unique_zone_shortcut_mapping.items():
...
def read_unique_zone_shortcuts_binary(
    ...
    for i in range(collection.EntriesLength()):
        ...
        unique_zone_shortcut_mapping[hex_id] = zone_id
