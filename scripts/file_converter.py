...
UniqueZoneShortcutMapping = Dict[int, int]
...
def compile_h3_map(candidates: Set) -> Tuple[ShortcutMapping, UniqueZoneShortcutMapping]:
    """
    ...
    unique_zone_mapping: UniqueZoneShortcutMapping = {}
    ...
    while candidates:
        ...
        # Check for unique zone
        if len(polys) == 1:
            unique_zone = poly_zone_ids_np[polys[0]]
            unique_zone_mapping[hex_id] = unique_zone
        elif len(polys) > 1:
            zones_in_cell = poly_zone_ids_np[polys]
            if len(np.unique(zones_in_cell)) == 1:
                unique_zone_mapping[hex_id] = zones_in_cell[0]
...
@time_execution
def compile_shortcut_mapping() -> Tuple[ShortcutMapping, UniqueZoneShortcutMapping]:
    """compiles h3 hexagon shortcut mapping and unique zone shortcut mapping

    returns: mapping from hexagon id to list of polygon ids,
             and mapping from hexagon id to unique zone id
...
def parse_data(
    ...
    # Write unique zone shortcuts
    unique_zone_shortcut_output_file = get_unique_zone_shortcut_file_path(output_path)
    write_unique_zone_shortcuts_flatbuffers(unique_zone_shortcuts, unique_zone_shortcut_output_file)
